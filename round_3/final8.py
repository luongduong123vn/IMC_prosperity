...
# === PARAMETERS ===
# Use the first algo’s parameters for RAINFOREST_RESIN, SQUID_INK, and KELP.
PARAMS = {
    ...
    # Use spread parameters from the new spread algo:
    Product.SPREAD: {
...
# Basket weights – these are used to build a synthetic basket from the components.
BASKET_WEIGHTS = {
    Product.JAMS: 3,
...
class Trader:
    def __init__(self, params=None):
        ...
        # Set limits for both core and basket products.
        self.LIMIT = {
    ...
    # --- Basic order functions (shared across products) ---
    ...
    # --- Order wrappers for each product ---
    ...
    def clear_orders(self, product: str, order_depth: OrderDepth, fair_value: float,
                     clear_width: int, position: int, buy_order_volume: int, sell_order_volume: int) -> (List[Order], int, int):
        orders: List[Order] = []
    ...
    # --- “Make” orders for the three core products (first-algo style) ---
    def make_rainforest_resin_orders(
        ...
        best_ask = min(order_depth.sell_orders.keys())
        best_bid = max(order_depth.buy_orders.keys())
        filtered_ask = [price for price in order_depth.sell_orders.keys() if price > fair_value + 1]
        filtered_bid = [price for price in order_depth.buy_orders.keys() if price < fair_value - 1]
        baaf = min(filtered_ask) if len(filtered_ask) > 0 else None
        bbbf = max(filtered_bid) if len(filtered_bid) > 0 else None
        if baaf == None or bbbf == None:
            baaf = best_ask - 1
            bbbf = best_bid + 1
        else:
    ...
    # --- Basket (synthetic) and spread functions (second-algo style) ---
    ...
    # --- Run method (merging core and basket trading) ---
    def run(self, state: TradingState):
        ...
        # Trade core products (first-algo style)
        ...
        if Product.RAINFOREST_RESIN in self.params and Product.RAINFOREST_RESIN in state.order_depths:
            ...
            resin_make_orders, _, _ = self.make_rainforest_resin_orders(
                state.order_depths[Product.RAINFOREST_RESIN],
                self.params[Product.RAINFOREST_RESIN]["fair_value"],
                resin_position, buy_order_volume, sell_order_volume,
        ...
        if Product.KELP in self.params and Product.KELP in state.order_depths:
            ...
            kelp_make_orders, _, _ = self.make_KELP_orders(
                state.order_depths[Product.KELP],
                kelp_fair_value,
                kelp_position, buy_order_volume, sell_order_volume,
        ...
        # --- Basket Arbitrage (Spread) Trading ---
        # Use the new spread trading function.
