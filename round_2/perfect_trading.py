...
class PerfectTrader:
    ...
    def optimize_trades(self) -> Dict[int, tuple]:
        """Optimize trades using dynamic programming on full dataset."""
        optimal_trades = {}
        ...
        for t_idx, timestamp in enumerate(self.timestamps):
            print(f"Processing timestamp {timestamp} ({t_idx + 1}/{len(self.timestamps)})")
            ...
            # Prune states to save memory
            if len(dp[t_idx + 1]) > 1000:  # Keep top 1000 states
                states_by_pnl = sorted(dp[t_idx + 1].items(), key=lambda x: x[1], reverse=True)[:1000]
                dp[t_idx + 1] = defaultdict(lambda: float('-inf'))
                for state, pnl in states_by_pnl:
                    dp[t_idx + 1][state] = pnl
        
        # Find best final state
        best_final_pnl = float('-inf')
        best_final_state = None
        
        for final_state in dp[len(self.timestamps)].keys():
            final_pnl = dp[len(self.timestamps)][final_state]
    ...
    def save_trades(self, optimal_trades: Dict[int, tuple]):
        ...
        # Save to JSON file
        with open('round_2/optimal_trades.json', 'w') as f:
            json.dump(trades_json, f, indent=2)
        
        print(f"\nGenerated {len(optimal_trades)} trades")
        print("Saved to optimal_trades.json")
